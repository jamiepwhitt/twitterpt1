...
public class UserController {
	...
	private void SetTweetCounts(List<User> users, Model model) {
		...
		for (User user : users) {
			List<TweetDisplay> tweets = tweetService.findAllByUser(user);
			tweetCounts.put((String) user.getUsername(), tweets.size());
		}
	...
	private void SetFollowingStatus(List<User> users, List<User> usersFollowing, Model model) {
		...
		String username = (String) userService.getLoggedInUser().getUsername();
		for (User user : users) {
			if (usersFollowing.contains(user)) {
				followingStatus.put((String) user.getUsername(), true);
			} else if (!user.getUsername().equals(username)) {
				followingStatus.put((String) user.getUsername(), false);
			}
		}
